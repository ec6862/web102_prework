...
const totalRaised = GAMES_JSON.reduce((count, game) => {
    return game.pledged + count;
},0);
...
function filterUnfundedOnly() {
    ...
    let listOfGames = GAMES_JSON.filter( (games) => {
...
let displayStr = `A total of ${totalRaised.toLocaleString('en-US')} has been raised for ${totalGames} games. 
    Currently, ${unFundedGames} ${unFundedGames > 1 ? "games remain" : "game remains"} unfunded. We need your help to fund these amazing games!`;

// create a new DOM element containing the template string and append it to the description container
let displayNewStr = document.createElement('p');
displayNewStr.innerHTML = displayStr;
descriptionContainer.appendChild(displayNewStr);
